...
class CaptureController {
  ...
  private hoverTarget(raw: EventTarget | null): HTMLElement | null {
    if (!(raw instanceof Element) || isDittoElement(raw)) return null;
  ...
  private onPointerUp(e: Event) {
    ...
    if (dx >= DRAG_MIN_PX || dy >= DRAG_MIN_PX) {
      ...
      if (!isDittoElement(target)) this.enqueue(this.capture('drag', target));
    }
  ...
  private onDragEnd(e: Event) {
    const target = eventTarget(e);
    if (!target || isDittoElement(target)) return;
