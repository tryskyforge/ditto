...
const EVENTS = ['ditto-blur:update-presets', 'ditto-blur:start-picker', 'ditto-blur:reset', 'ditto-blur:done'] as const;
...
export class BlurManager {
  ...
  private handleEvent = (e: Event) => {
    switch (e.type) {
      case 'ditto-blur:update-presets': {
      ...
      case 'ditto-blur:start-picker':
        this.picker.start(() => this.picker.stop());
        break;
      case 'ditto-blur:reset':
        this.scanner.unblurAll();
        break;
      case 'ditto-blur:done':
        this.teardown();
