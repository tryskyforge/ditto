...
const BLUR_CLASS = 'ditto-blur';
const BLUR_ATTR = 'data-ditto-blur';
const PEEK_CLASS = 'ditto-blur-peek';
...
export class BlurScanner {
  ...
  unblurAll() {
    ...
    const manuals = Array.from(document.querySelectorAll('.ditto-manual-blur'));
    for (const el of manuals) {
      el.classList.remove('ditto-manual-blur');
    }
  }
